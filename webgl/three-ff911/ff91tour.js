...
Mobile[0].size.w = 800;
// Battery
Mobile[1].camDist = 7.5;
Mobile[1].size.w = 800;
// Powertrain
Mobile[2].position = { x: 0, y: 2.8, z: 0.9 };
Mobile[2].camPos = { x: -1, y: 1.5, z: 1.2 };
Mobile[2].camDist = 6.5;
// Steering
Mobile[3].position = { x: 2, y: 2.8, z: -1.2 };
Mobile[3].camRot = { x: 95, y: 0 };
Mobile[3].camPos = { x: -1, y: 1.5, z: -1.5 };
Mobile[3].camDist = 6;
// Front Lighting
Mobile[4].camDist = 6.5;
Mobile[4].size.w = 800;
// Rear Lighting
Mobile[5].position = { x: 0, y: 3, z: 0 };
Mobile[5].camDist = 6.5;
Mobile[5].size.w = 800;
// Aerodynamics
Mobile[6].position = { x: 0, y: 3, z: 3 };
Mobile[6].camPos = { x: 0, y: 2, z: 0 };
Mobile[6].camDist = 10.5;
Mobile[6].size.w = 800;
// Free-viewing
Mobile[7].camDist = 8;
// Wide props are the same as desktop, with a few exceptions
var Wide = JSON.parse(JSON.stringify(Desktop));
exports.Wide = Wide;
// Battery
Wide[1].size.w = 1600;
Wide[1].size.h = 500;
...
var Controls = /** @class */ (function () {
    function Controls() {
        ...
        this.vp = new THREE.Vector2(document.documentElement.clientWidth, document.documentElement.clientHeight);
        ...
        document
            ...
            .appendChild(this.rendererWGL.domElement);
        // Camera setup
        var camOptions = {
    ...
    /////////////////////////////////////// NAV ////////////////////////////////////////
    // Clicked on nav item
    Controls.prototype.navClicked = function (_index, fireGAEvent) {
    ...
    Controls.prototype.backToFF = function () {
        ...
        //detect to web view in android and ios
        var isIOSWebView = /(iPhone|iPod|iPad).*AppleWebKit(?!.*Mobile)/i.test(navigator.userAgent);
        var isAndroidWebview = /Android/.test(navigator.userAgent) &&
            /Version\//.test(navigator.userAgent);
        if (isIOSWebView) {
            window["webkit"].messageHandlers["leaveWebview"].postMessage(null);
        }
        else if (isAndroidWebview) {
            window["android"]["leaveWebview"]();
        }
        else if (window["language"] === "us") {
            window.location.href = "https://www.ff.com/";
        }
        else {
            window.location.href = "https://www.faradayfuturecn.com/";
        }
    };
    Controls.prototype.outboundGA = function (action) {
        this.gA.uiEvent(action, "3DTour");
    };
    ...
    Controls.prototype.exitSection = function () {
        // Fire event for close button click
        this.gA.uiEvent("close button", "3DTour");
        // Tell nav to enter free driving
        this.nav.navClick(7, null, false);
    };
    /////////////////////////////////////// WINDOW LISTENERS ////////////////////////////////////////
    // User scrolls down
    ...
    Controls.prototype.checkIsMobile = function () {
        var isMobile = window.innerWidth <= 666;
        this.nav.isMobile = isMobile;
        return isMobile;
    };
    Controls.prototype.shouldShowNav = function () {
        var isMobile = this.checkIsMobile();
        if (isMobile) {
            this.nav.navVisible = false;
        }
        else {
            this.nav.navVisible = true;
            this.nav.showNav();
        }
    };
    ...
    Controls.prototype.onWindowResize = function () {
        ...
        this.vp.x = document.documentElement.clientWidth;
        this.vp.y = document.documentElement.clientHeight;
        this.rendererWGL.setSize(this.vp.x, this.vp.y);
...
var Batts = /** @class */ (function () {
    ...
    Batts.prototype.hide = function () {
        if (this.showing) {
            this.showing = false;
            TweenLite.killTweensOf(this);
            TweenLite.to(this.progUniform, 1, { value: 0, ease: Power2.easeInOut, onComplete: function () {
                    this.stringMesh.visible = false;
                }.bind(this) });
        }
    };
...
var Motors = /** @class */ (function () {
    ...
    Motors.prototype.hide = function () {
        if (this.showing) {
            this.showing = false;
            TweenLite.killTweensOf(this);
            TweenLite.to(this.progUniform, 1.0, { value: 0, ease: Power2.easeInOut, onComplete: function () {
                    this.group.visible = false;
                }.bind(this) });
        }
    };
...
var Card = /** @class */ (function () {
    ...
    Card.prototype.setSize = function (size) {
        this.foregDOM.style.width = size.w + "px";
        this.foregDOM.style.height = size.h + "px";
        this.backgDOM.style.width = (size.w + 40) + "px";
        this.backgDOM.style.height = (size.h + 40) + "px";
    };
    /////////////////////////////////////// PRIVATE METHODS ////////////////////////////////////////
...
var Nav = /** @class */ (function () {
    function Nav(_parent) {
        ...
        this.navShow.addEventListener("click", this.toggleNavBox.bind(this), false);
        // Exit buttons
        document
            .getElementById("reserveBut")
            .addEventListener("click", this.exitBtnClick.bind(this, 0), false);
        document
            .getElementById("Twitter")
            .addEventListener("click", this.exitBtnClick.bind(this, 1), false);
        document
            .getElementById("Facebook")
            .addEventListener("click", this.exitBtnClick.bind(this, 2), false);
        document
            .getElementById("LinkedIn")
            .addEventListener("click", this.exitBtnClick.bind(this, 3), false);
        document
            .getElementById("subscribeBut")
            .addEventListener("click", this.exitBtnClick.bind(this, 4), false);
    ...
    Nav.prototype.exitBtnClick = function (_i, evt) {
        ...
        switch (_i) {
            case 0:
                //support to web view in android and ios
                if (isIOSWebView) {
                    window["webkit"].messageHandlers["startReservation"].postMessage(null);
                }
                else if (isAndroidWebview) {
                    window["android"]["startReservation"]();
                }
                else if (lang === "us") {
                    url = "https://www.ff.com/us/reserve/";
                }
                else {
                    url = "https://www.faradayfuturecn.com/cn/reserve/";
                }
                labelGA = "reserve button";
                break;
            case 1:
                url =
                    "https://twitter.com/share?url=https://www.ff.com/us/3D-tour/&text=Check out the Faraday Future FF 91 3D Tour @FaradayFuture";
                labelGA = "twitter";
                break;
            case 2:
                url =
                    "https://www.facebook.com/sharer/sharer.php?u=https://www.ff.com/us/3D-tour/";
                labelGA = "facebook";
                break;
            case 3:
                url =
                    "https://www.linkedin.com/shareArticle?mini=true&url=https://www.ff.com/us/3D-tour/&title=Faraday Future FF 91 3D tour&summary=Check out the Faraday Future FF 91 3D Tour";
                labelGA = "linkedin";
                break;
            case 4:
                if (lang === "us") {
                    ...
                    url = "https://www.faradayfuturecn.com/cn/newsletter-subscribe/";
                }
                labelGA = "subscribe";
                break;
        }
    ...
    Nav.prototype.navClick = function (_index, ev, fireGAEvent) {
        if (fireGAEvent === void 0) { fireGAEvent = true; }
        ...
        this.parentControls.navClicked(_index, fireGAEvent);
    };
    Nav.prototype.toggleNavBox = function () {
        if (this.navVisible) {
            this.hideNav();
        }
        else {
    ...
    Nav.prototype.hideNav = function () {
    ...
    Nav.prototype.showNav = function () {
...
var ViewPreload = /** @class */ (function () {
    ...
    ViewPreload.prototype.exitAnimation = function (_callback) {
        TweenLite.to(this, 2.0, { progTarget: -0.2, ease: Power2.easeInOut, onComplete: function () {
...
var ViewTour = /** @class */ (function () {
    function ViewTour(_scene, _renderer, _cam, _vp) {
        ...
        this.viewType = this.findViewType(_vp.x, _vp.y);
        this.sectionPrev =
    ...
    ViewTour.prototype.findViewType = function (width, height) {
        if (width <= height * 1.2) {
            return 'MOBILE';
        }
    ...
    ViewTour.prototype.moveCamera = function (_cardProps) {
        ...
        TweenLite.to(this.cam, 2, { distTarget: _cardProps.camDist, onComplete: function () {
    ...
    ViewTour.prototype.initMeshes = function (_cargo) {
        ...
        if (this.viewType === 'MOBILE') {
            freeProps = CardProps.Mobile[7];
        }
        else if (this.viewType === 'WIDE') {
            freeProps = CardProps.Wide[7];
        }
        else {
            freeProps = CardProps.Desktop[7];
        }
    ...
    ViewTour.prototype.goToSection = function (index) {
        var sectProps;
        if (this.viewType === 'MOBILE') {
            sectProps = CardProps.Mobile[index];
        }
        else if (this.viewType === 'WIDE') {
            sectProps = CardProps.Wide[index];
        }
        else {
            sectProps = CardProps.Desktop[index];
        }
    ...
    ViewTour.prototype.onWindowResize = function (_vp) {
        this.rendererCSS.setSize(_vp.x, _vp.y);
        // No need to update card props/camera
        if (this.sectionActive === -1) {
            this.viewType = this.findViewType(_vp.x, _vp.y);
            return;
        }
        ...
        if (this.findViewType(_vp.x, _vp.y) === 'MOBILE' && this.viewType !== 'MOBILE') {
            this.viewType = 'MOBILE';
            this.moveCamera(CardProps.Mobile[this.sectionActive]);
            this.card.setSize(CardProps.Mobile[this.sectionActive].size);
            this.card.setPosition(CardProps.Mobile[this.sectionActive].position);
        }
        else if (this.findViewType(_vp.x, _vp.y) === 'WIDE' && this.viewType !== 'WIDE') {
            this.viewType = 'WIDE';
            this.moveCamera(CardProps.Wide[this.sectionActive]);
            this.card.setSize(CardProps.Wide[this.sectionActive].size);
            this.card.setPosition(CardProps.Wide[this.sectionActive].position);
        }
        else if (this.findViewType(_vp.x, _vp.y) === 'DESKTOP' && this.viewType !== 'DESKTOP') {
            this.viewType = 'DESKTOP';
            this.moveCamera(CardProps.Desktop[this.sectionActive]);
            this.card.setSize(CardProps.Desktop[this.sectionActive].size);
    ...
    AssetLoader.prototype.start = function () {
        ...
        var _loop_1 = function (i) {
            ...
            switch (this_1.manifesto[i].type) {
                case "texture":
                    this_1.loaderText.load(this_1.path + "textures/" + this_1.manifesto[i].name + ext, function (_obj) { this.assetAquired(_obj, this.manifesto[i].name); }.bind(this_1), undefined, function (_err) { this.assetFailed(_err, this.manifesto[i].name); }.bind(this_1));
                    break;
                case "mesh":
                    this_1.loaderMesh.load(this_1.path + "meshes/" + this_1.manifesto[i].name + ".json", function (_obj) { this.assetAquired(_obj, this.manifesto[i].name); }.bind(this_1), undefined, function (_err) { this.assetFailed(_err, this.manifesto[i].name); }.bind(this_1));
                    break;
                case "cubetexture":
